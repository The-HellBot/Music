...
async def favorites(_, message: Message):
    ...
    if message.command[0][1] == "d":
        delete = True
    await MakePages.favorite_page(hell, favs, 0, 0, True, delete)
...
async def myfavs_cb(_, cb: CallbackQuery):
    ...
    if action == "close":
        ...
        index = new_page * 7
        await MakePages.favorite_page(cb, collection, new_page, index, True)
...
async def delfavs_cb(_, cb: CallbackQuery):
    ...
    if action == "all":
        ...
        if is_deleted:
            ...
            collection = await db.get_all_favorites(int(user_id))
            await MakePages.favorite_page(cb, collection, 0, 0, True, True)
        else:
